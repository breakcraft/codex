...
- Run `pnpm run build` to produce the CLI bundle.
...
- The repository provides a [Nix flake](./flake.nix) with dev shells for both implementations. Run `nix develop .#codex-cli` or `nix develop .#codex-rs` to spawn an environment with all tools preinstalled.
- The `codex-rs/justfile` exposes handy commands like `just install` and `just tui` to build and run the Rust CLI.
- Utility scripts in `scripts/` verify documentation formatting:
  - `python3 scripts/asciicheck.py README.md` ensures `README.md` uses only ASCII or approved Unicode characters.
  - `python3 scripts/readme_toc.py README.md` updates the table of contents.
